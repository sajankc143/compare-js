...
let rightSpecies = null;
let comparisonOverlay = null;

// Function to create comparison overlay
function createComparisonOverlay() {
  ...
  closeBtn.onmouseover = () => {
    ...
    closeBtn.style.background = 'rgba(255, 68, 68, 0.9)';
  };
  closeBtn.onmouseout = () => {
  ...
  // Left search
  const leftSearch = createSearchSection('left', 'Left Species');
  // Right search  
  const rightSearch = createSearchSection('right', 'Right Species');

  searchSection.appendChild(leftSearch);
  searchSection.appendChild(rightSearch);
...
// Function to create search section for comparison
function createSearchSection(side, label) {
  const section = document.createElement('div');
  section.style.cssText = `
    border: 1px solid ${side === 'left' ? 'rgba(0, 123, 255, 0.4)' : 'rgba(40, 167, 69, 0.4)'};
    border-radius: 20px;
    padding: 25px;
    background: ${side === 'left' ? 'rgba(100, 149, 237, 0.15)' : 'rgba(144, 238, 144, 0.15)'};
    backdrop-filter: blur(8px);
    -webkit-backdrop-filter: blur(8px);
    box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1);
    transition: all 0.3s ease;
  ...
  titleDiv.style.cssText = `
    font-weight: bold;
    margin-bottom: 15px;
    color: ${side === 'left' ? 'rgba(173, 216, 230, 0.9)' : 'rgba(144, 238, 144, 0.9)'};
    font-size: 18px;
...
input.spellcheck = false;
  ...
  input.addEventListener('focus', () => {
    ...
    input.style.boxShadow = '0 2px 15px rgba(0, 0, 0, 0.2)';
    input.style.borderColor = side === 'left' ? 'rgba(0, 191, 255, 0.6)' : 'rgba(50, 205, 50, 0.6)';
  });
...
`;
  ...
  // Setup search suggestions for this input
  setupComparisonSearch(input.id, suggestionBox.id, side);

  return section;
}

// Function to setup comparison search suggestions
function setupComparisonSearch(inputId, suggestionBoxId, side) {
  ...
  input.addEventListener("input", async () => {
    ...
    try {
      ...
      const matches = Array.from(allSpeciesIncludingPng.entries())
        .filter(([normalizedName, originalName]) => normalizedName.includes(query))
        .map(([normalizedName, originalName]) => originalName);
      ...
      matches.slice(0, 8).forEach(name => {
        ...
        let formattedName;
        if (dashIndex !== -1) {
          const latin = name.substring(0, dashIndex);
          const common = name.substring(dashIndex + 3);
          formattedName = `<i>${latin}</i> - ${common}`;
        } else {
  ...
  input.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
      const firstSuggestion = suggestionBox.querySelector('div');
      if (firstSuggestion) {
...
// Function to load species data for comparison
async function loadSpeciesForComparison(speciesName, side) {
  ...
  panel.innerHTML = `
    <div style="text-align: center; padding: 50px;">
      <div style="color: rgba(255,255,255,0.8); font-size: 16px;">Loading ${speciesName}...</div>
    </div>
  `;
  ...
  try {
    ...
    if (!butterfly) {
      panel.innerHTML = `
        <div style="text-align: center; padding: 50px; color: rgba(255,100,100,0.9); font-size: 16px;">
    ...
    if (side === 'left') {
      leftSpecies = butterfly;
    } else {
      rightSpecies = butterfly;
    }
...
// Function to extract species details from HTML
...
links.forEach(link => {
  ...
  // Only accept image URLs ending with typical image extensions
  if (href && /\.(jpe?g|png|gif|webp|bmp)$/i.test(href) && !details.images.includes(href)) {
...
// Function to display species in comparison panel
function displaySpeciesInPanel(butterfly, details, panelId, side) {
  ...
  let html = `
    <div style="border-bottom: 1px solid ${borderColor}; padding-bottom: 15px; margin-bottom: 20px;">
      <h3 style="margin: 0; color: ${borderColor}; font-size: 20px; text-shadow: 0 1px 3px rgba(0, 0, 0, 0.3);">
        ${details ? details.scientificInfo.fullScientificName || butterfly.name : butterfly.name}
      </h3>
      ${details && details.scientificInfo.commonName ? 
        `<p style="margin: 5px 0 0 0; color: ${textColor}; font-size: 16px;">${details.scientificInfo.commonName}</p>` : ''}
    </div>
  `;
  ...
  if (details && details.images.length > 0) {
    html += `
      <div style="margin-bottom: 20px;">
        <h4 style="color: ${borderColor}; margin-bottom: 10px; text-shadow: 0 1px 3px rgba(0, 0, 0, 0.3);">Images</h4>
    ...
    details.images.slice(0, 4).forEach(src => {
      html += `
        ...
        " onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 6px 20px rgba(0, 0, 0, 0.3)'" 
          ...
          <img src="${src}" alt="Butterfly image" style="
  max-width: 100%;
  max-height: 100%;
  object-fit: contain;
  cursor: pointer;
...
onclick="openImageModal('${src}')">
  ...
  if (details && details.scientificInfo) {
    html += `
      <div style="margin-bottom: 20px;">
        <h4 style="color: ${borderColor}; margin-bottom: 10px; text-shadow: 0 1px 3px rgba(0, 0, 0, 0.3);">Scientific Classification</h4>
        <div style="background: ${accentColor}; padding: 15px; border-radius: 16px; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border: 1px solid rgba(255, 255, 255, 0.1);">
    `;
    
    if (details.scientificInfo.genus) {
      html += `<p style="margin: 5px 0; color: ${textColor};"><strong>Genus:</strong> <i>${details.scientificInfo.genus}</i></p>`;
    }
    if (details.scientificInfo.species) {
      html += `<p style="margin: 5px 0; color: ${textColor};"><strong>Species:</strong> <i>${details.scientificInfo.species}</i></p>`;
    }
  ...
  html += `
    <div style="margin-bottom: 20px;">
      <h4 style="color: ${borderColor}; margin-bottom: 10px; text-shadow: 0 1px 3px rgba(0, 0, 0, 0.3);">Availability</h4>
      <div style="background: ${accentColor}; padding: 15px; border-radius: 16px; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border: 1px solid rgba(255, 255, 255, 0.1);">
        <p style="margin: 5px 0; color: ${textColor};"><strong>Found on:</strong> ${butterfly.origins.length} page(s)</p>
        <p style="margin: 5px 0; color: ${textColor};"><strong>On Homepage:</strong> ${butterfly.onHomepage ? 'Yes' : 'No'}</p>
        ${butterfly.isPng ? `<p style="margin: 5px 0; color: rgba(255, 165, 0, 0.9);"><strong>Note:</strong> PNG species</p>` : ''}
      </div>
    </div>
  `;
  ...
  html += `
    <div>
      <h4 style="color: ${borderColor}; margin-bottom: 10px; text-shadow: 0 1px 3px rgba(0, 0, 0, 0.3);">View on Site</h4>
  ...
  butterfly.urls.slice(0, 3).forEach((url, index) => {
    ...
    html += `
      ...
      " onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 15px rgba(0, 0, 0, 0.3)'"
        onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 10px rgba(0, 0, 0, 0.2)'">${pageName}</a>
    `;
  });
...
// Function to open comparison overlay
...
// Function to close comparison overlay
...
// Function to create comparison button
...
function initializeComparison() {
  ...
  // Inject glassmorphic style overrides for ultra-transparent viselike effect
(function injectGlassmorphicOverrides() {
  const style = document.createElement('style');
  style.innerHTML = `
  #speciesComparisonOverlay h2 {
  font-size: 22px !important;
}
#speciesComparisonOverlay h3 {
  font-size: 16px !important;
}
#speciesComparisonOverlay h4 {
  font-size: 14px !important;
}
#speciesComparisonOverlay p {
  font-size: 13px !important;
}

    #speciesComparisonOverlay,
    #speciesComparisonOverlay > div,
    #comparisonDisplay > div,
    #comparisonDisplay .img-container,
    #comparisonDisplay input,
    #comparisonDisplay .suggestion-box,
    #comparisonLeftSuggestions,
    #comparisonRightSuggestions,
    #openComparisonBtn,
    button,
    input {
    ...
    #comparisonLeftSuggestions div:hover,
    #comparisonRightSuggestions div:hover {
      background: rgba(0, 150, 255, 0.3) !important;
      color: white !important;
    }

    #speciesComparisonOverlay img {
